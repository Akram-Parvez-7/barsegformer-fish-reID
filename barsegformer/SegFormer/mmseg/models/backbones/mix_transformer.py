...
# ---------------------------------------------------------------
# Modified for BarSegFormer:
#   - Stage 1 patch embedding replaced with StripeAwareStem
#   - 3x3 depthwise conv added after each encoder stage
#     for explicit local feature extraction
# ---------------------------------------------------------------
import torch
...
from .stripe_stem import StripeAwareStem
...
class LocalDWConv(nn.Module):
    """
    Explicit local feature extraction module.
    3x3 depthwise conv added after each encoder stage
    to capture fine textures and stripe edges.
    """
    ...
    def forward(self, x):
        # x is (B, C, H, W) at this point (after reshape in forward_features)
        return x + self.act(self.bn(self.dwconv(x)))


class OverlapPatchEmbed(nn.Module):
    """ Image to Patch Embedding — kept for stages 2, 3, 4 """
...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000,
        ...
        # Stage 1: replaced with StripeAwareStem
        self.patch_embed1 = StripeAwareStem(
            in_chans=in_chans,
            embed_dim=embed_dims[0],
            stride=4
        )

        # Stages 2-4: keep original OverlapPatchEmbed
        ...
        # Local feature extraction modules after each stage
        self.local_conv1 = LocalDWConv(embed_dims[0])
        self.local_conv2 = LocalDWConv(embed_dims[1])
        self.local_conv3 = LocalDWConv(embed_dims[2])
        self.local_conv4 = LocalDWConv(embed_dims[3])
    ...
    def forward_features(self, x):
        ...
        # Stage 1 — stripe-aware stem + transformer blocks + local conv
        x, H, W = self.patch_embed1(x)
        for i, blk in enumerate(self.block1):
            x = blk(x, H, W)
        x = self.norm1(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x = self.local_conv1(x)  # explicit local feature extraction
        outs.append(x)
