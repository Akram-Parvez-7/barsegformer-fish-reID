...
# ── RGB-only transform (mask ignored) ──────────────────────────────────────────
class SyncTransformRGB:
    """Same augmentations as SyncTransform, but outputs RGB only (3 channels)."""
    ...
    def __call__(self, image, mask):
        # mask is accepted but ignored entirely
...
class CichlidDatasetSync(CichlidDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.sync_transform:
            ...
            combined = TF.to_tensor(TF.resize(image, (224, 224)))
...
def main():
    ...
    train_sync = SyncTransformRGB(image_size=224, is_train=True)
    val_sync = SyncTransformRGB(image_size=224, is_train=False)
    ...
    # Model — in_channels=3 for RGB only
    model = CichlidIDModel(num_classes=NUM_CLASSES, in_channels=3).to(device)
