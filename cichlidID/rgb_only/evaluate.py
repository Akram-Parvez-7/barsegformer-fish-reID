...
from train import CichlidDatasetSync, SyncTransformRGB
from model import CichlidIDModel


def load_model(checkpoint_path, num_classes=25, device="cuda"):
    checkpoint = torch.load(checkpoint_path, map_location=device)
    model = CichlidIDModel(num_classes=num_classes, in_channels=3)
...
def plot_confusion_matrix(labels, preds, label_map, save_path):
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",
                ...
                linewidths=0.5)
    plt.title("Confusion Matrix — Cichlid Individual ID (RGB only)", fontsize=14, pad=20)
...
def plot_per_fish_accuracy(labels, preds, label_map, save_path):
    ...
    plt.title("Per-Fish Identification Accuracy (RGB only)", fontsize=14)
